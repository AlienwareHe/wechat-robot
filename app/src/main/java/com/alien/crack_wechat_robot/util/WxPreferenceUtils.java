...
public class WxPreferenceUtils {
    ...
    public static String getWxId() {
        SharedPreferences sharedPreferences = SharedObject.context.getSharedPreferences("com.tencent.mm_preferences", Context.MODE_PRIVATE);
        try {
            if (TextUtils.isEmpty(WX_ID)) {
                ...
                if (TextUtils.isEmpty(WX_ID)) {//全局搜索 login_weixin_username，可以看到存储的地方
                    Class G = RposedHelpers.findClass("com.tencent.mm.kernel.g", SharedObject.loadPackageParam.classLoader);
